...
case class Database(tables : List[Table]) {
   ...
   def insertInTable( objectToSave: Serializable, tableName: String) = Database.insertInTable(this, objectToSave, tableName)
   def swapTable(tableName: String, newTable: Table) = Database.swapTable(this, tableName, newTable)
   def getTableByName(tableName: String) = Database.getTableByName(this, tableName)
}
...
object Database{
   ...
   def insertInTable(database: Database, objectToSave: Serializable, tableName: String): Database =
   {
      ...
      new Database(database.tables.updated(database.tables.indexOf(database.getTableByName(tableName)), table.insert(objectToSave: Serializable)))
   }
